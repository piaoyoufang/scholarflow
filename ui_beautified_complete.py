...
def render_learning_plan_tab() -> None:
    """Tab 4：学习计划 Agent。"""
    render_section_header("学习计划", "输入学习目标，由 Agent 按天拆解任务、产出和难度节奏。", "🗓️")
    course_id = require_selected_course()
    if not course_id:
        return
...
def render_tasks_tab() -> None:
    ...
    render_section_header("上传任务", "查看课程资料入库任务状态，快速定位解析或向量化进度。", "⚙️")
    course_id = require_selected_course()
    if not course_id:
        return
...
st.set_page_config(page_title="ScholarFlow｜AI课程知识库与学习助手", page_icon="🎓", layout="wide")
apply_theme()
