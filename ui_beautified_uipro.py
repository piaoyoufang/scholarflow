...
:root {
    ...
    --sf-shadow: 0 14px 40px rgba(15, 23, 42, 0.08);
    --sf-shadow-soft: 0 8px 24px rgba(15, 23, 42, 0.06);
    --sf-radius-lg: 22px;
    --sf-radius-md: 16px;
}
.stApp {
    background: radial-gradient(circle at 12% 8%, rgba(37, 99, 235, 0.08), transparent 28rem), linear-gradient(180deg, #fbfcff 0%, var(--sf-bg) 52%, #f3f4f6 100%);
    color: var(--sf-text);
}
.block-container { max-width: 1180px; padding-top: 1.4rem; padding-bottom: 3.5rem; }
[data-testid="stHeader"] { background: transparent; }
[data-testid="stSidebar"] { background: #0f172a; border-right: 1px solid rgba(255, 255, 255, 0.08); }
[data-testid="stSidebar"] * { color: rgba(255,255,255,.92) !important; }
[data-testid="stSidebar"] .stCaptionContainer, [data-testid="stSidebar"] [data-testid="stMarkdownContainer"] p { color: rgba(226,232,240,.72) !important; }
[data-testid="stSidebar"] code { display: block; padding: 10px 12px !important; border-radius: 12px !important; background: rgba(255,255,255,.08) !important; border: 1px solid rgba(255,255,255,.10) !important; color: #dbeafe !important; }
.sf-hero { position: relative; padding: 30px 32px; margin-bottom: 18px; border-radius: 28px; background: linear-gradient(135deg, #ffffff 0%, #f8fbff 100%); border: 1px solid rgba(226, 232, 240, 0.95); box-shadow: var(--sf-shadow); overflow: hidden; }
.sf-hero:before { content: ""; position: absolute; right: -80px; top: -90px; width: 260px; height: 260px; border-radius: 999px; background: radial-gradient(circle, rgba(37,99,235,.16), transparent 68%); }
.sf-eyebrow { position: relative; display: inline-flex; align-items: center; gap: 7px; padding: 6px 11px; border-radius: 999px; background: var(--sf-primary-soft); color: #1d4ed8; border: 1px solid #dbeafe; font-size: 13px; font-weight: 650; }
.sf-title { position: relative; margin: 14px 0 10px; color: #0f172a; font-size: clamp(34px, 4vw, 50px); line-height: 1.06; font-weight: 850; letter-spacing: -0.045em; }
.sf-subtitle { position: relative; max-width: 760px; margin: 0; color: var(--sf-muted); font-size: 16px; line-height: 1.8; }
.sf-chip-row { position: relative; display: flex; flex-wrap: wrap; gap: 10px; margin-top: 20px; }
.sf-chip { padding: 7px 11px; border-radius: 999px; color: #334155; background: #ffffff; border: 1px solid var(--sf-border); box-shadow: 0 4px 14px rgba(15,23,42,.04); font-size: 13px; }
.sf-panel, .sf-login-card, .sf-card { padding: 22px; border-radius: var(--sf-radius-lg); background: rgba(255,255,255,.92); border: 1px solid rgba(226,232,240,.95); box-shadow: var(--sf-shadow-soft); }
.sf-panel { margin-bottom: 16px; }
.sf-section-title { margin: 12px 0 8px; color: #111827; font-size: 23px; font-weight: 800; letter-spacing: -0.025em; }
.sf-section-desc { margin: 0; color: var(--sf-muted); font-size: 14.5px; line-height: 1.75; }
.sf-course-card { padding: 14px 16px; margin: 8px 0 18px; border-radius: var(--sf-radius-md); background: #ffffff; border: 1px solid var(--sf-border); box-shadow: var(--sf-shadow-soft); }
.sf-status-pill { display: inline-flex; align-items: center; gap: 8px; padding: 8px 12px; border-radius: 999px; background: var(--sf-primary-soft); color: #1d4ed8; border: 1px solid #dbeafe; font-weight: 700; font-size: 13px; }
.sf-empty-hint { padding: 18px 20px; border-radius: var(--sf-radius-md); background: #ffffff; border: 1px dashed #cbd5e1; color: #475569; box-shadow: 0 6px 18px rgba(15,23,42,.035); }
.sf-doc-card { padding: 16px 18px; border-radius: var(--sf-radius-md); background: #ffffff; border: 1px solid var(--sf-border); box-shadow: 0 8px 22px rgba(15,23,42,.05); margin-bottom: 12px; }
.sf-doc-title { font-weight: 760; color: #111827; margin-bottom: 6px; }
.sf-doc-meta { color: #64748b; font-size: 13px; line-height: 1.7; }
h1, h2, h3 { letter-spacing: -0.025em; }
hr { margin: 1.35rem 0; border-color: rgba(203,213,225,.7); }
small, .stCaptionContainer { color: var(--sf-muted) !important; }
[data-testid="stTabs"] [data-baseweb="tab-list"] { gap: 6px; padding: 6px; border-radius: 16px; background: #ffffff; border: 1px solid var(--sf-border); box-shadow: var(--sf-shadow-soft); }
[data-testid="stTabs"] [data-baseweb="tab"] { height: 42px; padding: 0 14px; border-radius: 12px; color: #475569; font-weight: 680; }
[data-testid="stTabs"] [aria-selected="true"] { background: #0f172a; color: #ffffff !important; }
.stButton > button, .stDownloadButton > button, button[kind="primary"] { min-height: 42px; border-radius: 13px !important; border: 1px solid #1d4ed8 !important; background: #2563eb !important; color: #ffffff !important; font-weight: 720 !important; box-shadow: 0 9px 22px rgba(37,99,235,.20) !important; transition: transform .15s ease, box-shadow .15s ease, background .15s ease; }
.stButton > button:hover, .stDownloadButton > button:hover { transform: translateY(-1px); background: #1d4ed8 !important; box-shadow: 0 12px 26px rgba(37,99,235,.25) !important; }
.stButton > button[kind="secondary"] { background: #ffffff !important; color: #334155 !important; border: 1px solid #dbe3ef !important; box-shadow: 0 7px 18px rgba(15,23,42,.05) !important; }
...
[data-testid="stTextInput"] input, [data-testid="stTextArea"] textarea, [data-testid="stSelectbox"] div[data-baseweb="select"] > div, [data-testid="stNumberInput"] input { border-radius: 13px !important; border-color: #dbe3ef !important; background: #ffffff !important; box-shadow: 0 4px 14px rgba(15,23,42,.035) !important; }
[data-testid="stTextInput"] input:focus, [data-testid="stTextArea"] textarea:focus { border-color: #93c5fd !important; box-shadow: 0 0 0 4px rgba(37,99,235,.10) !important; }
[data-testid="stFileUploader"] section { border-radius: var(--sf-radius-md) !important; border: 1px dashed #bfdbfe !important; background: #f8fbff !important; }
[data-testid="stForm"], [data-testid="stExpander"], [data-testid="stDataFrame"], .stAlert { border-radius: var(--sf-radius-md) !important; }
[data-testid="stDataFrame"] { overflow: hidden; border: 1px solid var(--sf-border); box-shadow: 0 8px 22px rgba(15,23,42,.045); }
[data-testid="stMetric"] { padding: 18px; border-radius: var(--sf-radius-md); background: #ffffff; border: 1px solid var(--sf-border); box-shadow: var(--sf-shadow-soft); }
[data-testid="stMetricValue"] { color: #0f172a; font-weight: 850; }
[data-testid="stMetricLabel"] { color: #64748b; }
[data-testid="stChatMessage"] { border-radius: 18px; padding: 12px 14px; background: #ffffff; border: 1px solid var(--sf-border); box-shadow: 0 8px 22px rgba(15,23,42,.045); }
[data-testid="stChatInput"] textarea { border-radius: 16px !important; border: 1px solid #dbe3ef !important; box-shadow: 0 10px 28px rgba(15,23,42,.08) !important; }
...
.stAlert { border: 1px solid var(--sf-border) !important; box-shadow: 0 8px 20px rgba(15,23,42,.045); }
*:focus-visible { outline: 3px solid rgba(37,99,235,.26) !important; outline-offset: 2px !important; }
@media (max-width: 900px) {
    .sf-feature-grid { grid-template-columns: repeat(2, minmax(0, 1fr)); }
    .sf-hero { padding: 24px; border-radius: 22px; }
}
...
def render_hero(title: str, subtitle: str, eyebrow: str = "AI Knowledge Base", chips: list[str] | None = None) -> None:
    """Render the unified product hero area."""
    chips = chips or ["RAG 问答", "课程知识库", "学习计划", "自动出题", "教师分析"]
...
def render_feature_grid() -> None:
    ...
    st.markdown(
        ...
        <div class="sf-feature-grid">
            <div class="sf-feature-card"><div class="sf-feature-title">RAG 课程问答</div><div class="sf-feature-desc">围绕课程资料生成可追溯答案，降低幻觉风险。</div></div>
            <div class="sf-feature-card"><div class="sf-feature-title">资料自动入库</div><div class="sf-feature-desc">支持 PDF、Markdown、TXT 上传，异步解析与向量化。</div></div>
            <div class="sf-feature-card"><div class="sf-feature-title">学习 Agent</div><div class="sf-feature-desc">根据学习目标拆解计划、练习题与每日产出。</div></div>
            <div class="sf-feature-card"><div class="sf-feature-title">教师分析看板</div><div class="sf-feature-desc">统计高频、无引用和低质量问题，形成优化闭环。</div></div>
        </div>
        """,
        unsafe_allow_html=True,
    )
...
def render_status_callout(message: str) -> None:
    ...
    st.markdown(
        f'<div class="sf-callout"><span class="sf-status-dot"></span>{message}</div>',
        unsafe_allow_html=True,
    )
...
def render_login_page() -> None:
    ...
    render_hero(
        "ScholarFlow",
        "把课程资料变成可追问、可引用、可评估的 AI 学习助手。支持课程知识库、RAG 问答、学习计划、自动出题和教师分析闭环。",
        eyebrow="Welcome Back",
        chips=["课程知识库", "RAG 溯源问答", "Agent 学习计划", "教师分析看板"],
    )
    st.markdown('<div class="sf-login-card">', unsafe_allow_html=True)
    st.markdown(
        '<h2 class="sf-section-title">账号登录</h2><p class="sf-section-desc">登录后继续管理课程资料和 AI 问答会话。</p>',
        unsafe_allow_html=True,
    )
