...
def generate_learning_plan(
    ...
    """
    # RAG检索：以学习目标作为query，只检索当前course_id课程知识库，召回8条相关文档块
    docs = search(query=goal, course_id=course_id, k=8)

    # 遍历检索结果，取出每一个Document的page_content文本，使用双换行拼接成完整参考上下文
    # _score：下划线代表丢弃相关性分数，此处不需要使用分数
    context = "\n\n".join([doc.page_content for doc, _score in docs])

    # 获取大模型实例，开启结构化输出，强制LLM输出解析为LearningPlanResponse模型
    model = chat_model().with_structured_output(LearningPlanResponse)
